...
namespace chainsql
{
	...
	public:
		...
		virtual SqlChangeInfo makeUpdateStmtChange(ChainsqlUpdateStmt *stmt, const SqlTableColumns &columns, const std::vector<std::pair<row_id_type, SqlRecordData>> &update_before_rows);
		virtual SqlChangeInfo makeDeleteStmtChange(ChainsqlDeleteStmt *stmt, const SqlTableColumns &columns, const std::vector<std::pair<row_id_type, SqlRecordData>> &delete_before_rows);
